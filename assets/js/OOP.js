...
function wordsList(str, subStr) {
    ...
    let newLongStr = str.replace(/[^a-zа-яё\s]/gi, "").toLowerCase().split(" ");

    newLongStr.forEach((value) => {
...
function getLocalDate(date, isSeconds, isISO) {
    ...
    if (isISO !== undefined && isISO !== false) {
        rDate = `${year}-${month}-${day}, ${hour}:${minutes}`;
    }else {
        rDate = `${day}.${month}.${date.getFullYear()}, ${hour}:${minutes}`;
    }

    if (isSeconds !== undefined && isSeconds !== false) {
        rDate += `:${seconds}`;
    }
...
function getDateAgo(date, days) {
    ...
    const num = day.getMonth() < 9 ? "0" : "";

    return `${day.getDate()}.${num}${day.getMonth() + 1}.${day.getFullYear()}`;
}

// console.log(getDateAgo('2019-10-29', 1)); // 28.01.2019
